...
public class CacheAspect {
     ...
     * @see CacheData
     */
    @Pointcut("@annotation(com.fwr.redis.annotation.CacheData)")
    ...
    public Object logAround(ProceedingJoinPoint pjpParam, CacheData cacheData) throws Throwable {
        ...
        try {
            ...
            //解锁
            lock.unlock();
            log.info("[ CacheAspect ] >> un lock method:{} , cacheKey:{}, totalCost:{}", className + methodName, cacheKey, System.currentTimeMillis() - start);
        }
    }
    ...
    private String getCacheKey(CacheData cacheData, String className, String methodName, ProceedingJoinPoint pjpParam) {
        ...
        if (pjpParam.getArgs() == null || pjpParam.getArgs().length == 0) {
            if (EMPTY.equals(keyPrefix)) {
                return CACHE_KEY_PREFIX + methodPath;
            }
            return CACHE_KEY_PREFIX + keyPrefix + POINT + methodPath;
        }
        //参数序号
        int i = 0;
        //按照参数顺序,拼接方法参数
        ...
        if (EMPTY.equals(keyPrefix)) {
            return CACHE_KEY_PREFIX + methodPath + paramJson;
        }
        return CACHE_KEY_PREFIX + keyPrefix + POINT + methodPath + paramJson;
    }
